...
<div class="sr-narrow sr-content sr-formsection" sroperator="AND" srtype="<?php echo $get_vars->t; ?>">
    <div class="progress">
    ...
    if (isset($get_vars->o) && is_array($get_vars->o)) {
        ...
        foreach ($get_vars->o as $order) {
            ?>
            <input type="text" style="display: none" class="sr-order" srfield="<?= $order->f; ?>"
                   srdirection="<?= $order->o == 0 ? "DESC" : "ASC"; ?>">
            <?php

        }
    }

    foreach ($conditions['conditions'] as $key => $cod) {
        if (!empty($conditions['conditions'][$key]['conditions'])) {
            ?>
            <select style="display: none" class="sr-formelement"
                    srfield="<?= $conditions['conditions'][$key]['conditions'][0]['field']; ?>" sroperator="LIKE"
                    srloose="1"
                    multiple="" name="" id="">
                <?php
                foreach ($conditions['conditions'][$key]['conditions'] as $icod) {
                    ?>
